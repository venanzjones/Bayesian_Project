...
transformed data {
  matrix[nstations,nstations] H = exp(-(1/phi^2) * distances);
}
...
model {
  beta ~ normal(1, 2);
  for (i in 1:N) {
    y[i] ~ poisson(lambda[i]) T[, U];
  }

  xi ~ normal(1, 2);
  eta ~ normal(1, sigma0);
  w ~ multi_normal_cholesky(rep_vector(1, nstations), Lw);
  sigma ~ inv_gamma(2, 2);
...
generated quantities {
  ...
  for (i in 1:N_miss) {
    ...
    real u = uniform_rng(0, 1);
    for (b in 0:max_month[i]) {
      sum_p = sum_p + exp(poisson_lpmf(b | lambda_miss[i]) - poisson_lcdf(max_month_miss[i] | lambda_miss[i]));
      if (sum_p >= u) {
        y_pred_miss[i] = b;
        break;
      }

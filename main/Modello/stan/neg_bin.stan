...
model {
  beta ~ normal(0, 2);
  y[1:N] ~ neg_binomial_2(lambda[1:N], phi_bin);

  phi_bin ~ lognormal(0, 2);
...
generated quantities {
  ...
  */
  ...
  for(i in 1:N){
    log_lik [ i ] = neg_binomial_2_lpmf (y[i] | lambda[i], phi_bin);
    y_pred[i] = neg_binomial_2_rng(lambda[i], phi_bin);
  }

  for(i in 1:N_miss){
    y_pred_miss[i] = neg_binomial_2_rng(lambda_miss[i], phi_bin);
  }
}